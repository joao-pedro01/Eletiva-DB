...
SELECT 	C.nome, C.idCidade, C.email, D.nome
FROM cliente C
INNER JOIN cidade D
...
SELECT 	C.nome, C.idCidade, C.email, c.idEstado, D.nome, E.nome 
FROM cliente C
INNER JOIN cidade D
ON C.idCidade = D.idCidade
INNER JOIN  estado E
...
SELECT p.idPedido, p.data_pedido, c.nome
FROM pedido P
...
SELECT p.idPedido, p.data_pedido, c.nome, v.nome  'nome vendedor'
FROM pedido P
INNER JOIN cliente C
ON c.idCliente = p.idCliente
...
SELECT d.idCliente, p.nome, p.preco
FROM itens_pedido ip
INNER JOIN produto p
ON ip.idProduto = p.idProduto
INNER JOIN pedido d
ON ip.idPedido = d.idPedido
WHERE d.idCliente = 1;

## ex9

SELECT p.idPedido, p.idCliente, c.nome
FROM pedido p
INNER JOIN cliente c
ON c.idCliente = p.idCliente
...
select p.nome, p.preco, m.nome, c.nome
from produto p
inner join marca m
on p.idmarca = m.idmarca
inner join categoria c
...
SELECT d.idPedido, ip.valor_total, c.nome
FROM itens_pedido ip
INNER JOIN pedido d
ON ip.idPedido = d.idPedido
INNER JOIN cliente c
ON c.idCliente = d.idCliente
order by  ip.valor_total desc limit 1;
...
INNER JOIN cliente c
ON c.idCliente = d.idCliente
order by  ip.valor_total limit 1;